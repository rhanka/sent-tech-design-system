...
- [ ] **M2 — Fondations**
  ...
  - [ ] `GD-M2-WORKERS` : snapshot/version/cancel/fallback/rejet périmé et résolution worker dans pack. Cadrage écrit : [SPEC_EVOL_GD_M2_WORKERS.md](../spec/SPEC_EVOL_GD_M2_WORKERS.md) ; ce n'est pas un rapatriement — l'amont a 20 lignes de worker et 85 de client, sans version de snapshot, sans annulation et sans rejet du périmé, soit les quatre propriétés demandées. Motif mesuré : `computeLayout` prend 11 490 ms à 20 000 nœuds depuis #77.
  ...
  - [ ] `GD-M2-PARITY` et `GD-M2-THEMES` : quatre frameworks, a11y/IME/SSR et géométrie/token communes. Retard mesuré : `dataviz-angular` expose 76 composants contre 118 en Svelte, 118 en React et 117 en Vue (union des trois : 119) ; lots 1 à 4 livrés avec le patron de portage (`packages/dataviz-angular/PATTERN.md`) et le générateur `tools/dataviz-angular-port`, 43 adaptateurs restants, tous refusés par le générateur (surtout des panneaux à état) : le levier descripteur est épuisé, la suite est à écrire à la main.
    - [ ] `NG-A11Y-FORCEGRAPH` : porter dans `packages/components-angular/src/ForceGraph.ts` les quatre attributs du nœud React (`tabIndex=0`, `role="button"`, `aria-label`, `aria-pressed`), la gestion `focus`/`blur`/`keydown` qui les rend opérables, et le chemin d'accroche par arête `st-forceGraph__edgeHit` (`role="presentation"`). Déficit antérieur à l'adaptateur, mais exposé par lui : nœuds inatteignables au clavier et non annoncés. Coût mesuré : 42 entrées markup et 8 entrées de signature sur `ForceGraph`, égales à son témoin DS nu — donc résidu entièrement dans `components-angular`. Acceptation = `npm run parity:dataviz-angular` ramène `ForceGraph` à 0/0 et son témoin avec lui. Détail et table de portage : `packages/dataviz-angular/PATTERN.md`, section « Known debt ».
