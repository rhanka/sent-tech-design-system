<script lang="ts">
  ...
  // Version adaptateur : même contrat direct, câblée sur un vrai store minimal.
  ...
  const store = createDashboardStore({
    model: storeModel,
    data: [
      { product: "Atlas", revenue: 72 },
      { product: "Beacon", revenue: 28 }
    ]
  });
  ...
  const storeDemo = $derived<NodeSpec[]>(
    storeChartDemoNodes("GaugeChart", {
      ...
      max: 100,
      unit: "%",
      label: locale.value === "fr" ? "Charge (store)" : "Load (store)"
    })
  );
</script>
  ...
  <section class="docs-section">
    ...
    <table class="docs-table">
      ...
      <tbody>
        ...
        <tr><td><code>size</code> / <code>thickness</code></td><td><code>number</code></td><td>natif</td></tr>
        <tr><td><code>startAngle</code> / <code>endAngle</code></td><td><code>number</code></td><td>natif</td></tr>
        <tr><td><code>label</code></td><td><code>string</code></td><td>non défini</td></tr>
      </tbody>
    </table>
    <p class="docs-demo-context">
      ...
      {:else}
        Native props not carried by the store version: none.
      {/if}
    </p>
  </section>
</div>
