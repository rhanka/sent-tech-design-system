<script lang="ts">
  ...
  const copy = {
    fr: {
      ...
      basicTitle: "Bande lissée",
      basicDesc: "Bande remplie entre low et high sur un axe X ordinal, contours haut et bas en courbes spline.",
      stockTitle: "Min/max boursier",
      stockDesc: "Une fourchette mensuelle min/max avec contours lissés et une autre teinte.",
    ...
    en: {
      ...
      badge: "Documented",
      intro:
        "AreaSplineRangeChart draws a filled band between a low and a high series along a continuous X axis, with always-smoothed (curved) top and bottom edges. The spline variant of AreaRange, ideal for gentle ranges: min/max temperatures, confidence intervals, stock min/max.",
      examplesTitle: "Examples",
      basicTitle: "Smoothed band",
      basicDesc: "Filled band between low and high over an ordinal X axis, with curved spline top and bottom contours.",
      stockTitle: "Stock min/max",
      stockDesc: "A monthly min/max range with smoothed edges and a different hue.",
      ...
      usageNote:
        "Provide `low` and `high` per point; order is normalized automatically. Spline smoothing is always on. The X axis is numeric when all `x` are finite numbers, otherwise ordinal.",
      tokensTitle: "Tokens"
    }
  } as const;
      ...
      children: [
        {
          ...
          props: {
            label:
              locale.value === "fr"
                ? "Fourchette de température par jour (lissée)"
                : "Daily temperature range (smoothed)",
            tone: "category1",
            data: tempData
          }
        }
  ...
  const stockDemo = $derived<NodeSpec[]>([
    {
      el: "div",
      props: { class: "chart-wrapper" },
      children: [
...
<div class="docs-page">
  ...
  <section class="docs-section">
    ...
    <h3 class="docs-demo-title">{text().stockTitle}</h3>
    <p class="docs-demo-note">{text().stockDesc}</p>
    <TabbedExample nodes={stockDemo} title={text().stockTitle} />
  </section>
  ...
  <section class="docs-section">
    ...
    <table class="docs-table">
      ...
      <tbody>
        <tr><td><code>data</code></td><td><code>AreaSplineRangeChartDatum[]</code></td><td><em>{text().required}</em></td></tr>
    ...
    <p class="docs-demo-note">
      <code>AreaSplineRangeChartDatum</code> = <code>{`{ x: string | number; low: number; high: number }`}</code>
      ·
      <code>AreaSplineRangeChartTone</code> = <code>"category1" | ... | "category8"</code>.
    </p>
  </section>
