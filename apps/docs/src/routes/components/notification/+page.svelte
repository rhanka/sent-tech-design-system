<script lang="ts">
  ...
  const tonesNodes: NodeSpec[] = $derived([
    {
      el: "div",
      props: { class: "fp-stack" },
      children: [
        { comp: "Notification", props: { tone: "info", title: fr("Info", "Info"), message: fr("Une version mineure a été publiée.", "A minor release was published.") } },
        { comp: "Notification", props: { tone: "success", title: fr("Sauvegardé", "Saved"), message: fr("Les modifications ont été prises en compte.", "Your changes were applied.") } },
        { comp: "Notification", props: { tone: "warning", title: fr("Attention", "Warning"), message: fr("Le quota approche des limites.", "Quota is nearing limits.") } },
        { comp: "Notification", props: { tone: "error", title: fr("Échec", "Error"), message: fr("Le chargement a échoué.", "Loading failed.") } }
      ]
    }
  ]);

  const actionsNodes: NodeSpec[] = $derived([
    {
      el: "div",
      props: { class: "fp-stack" },
      children: [
        {
          ...
          children: [
            { comp: "Button", props: { variant: "ghost" }, children: [fr("Réessayer", "Retry")] }
          ]
        }
      ]
    }
  ...
  const dismissNodes: NodeSpec[] = $derived([
    {
      el: "div",
      props: { class: "fp-stack" },
      children: [
...
<div class="docs-page">
  ...
  <TabbedExample nodes={getExample("notification")?.nodes ?? []} />
  ...
  <section class="docs-section">
    ...
    <TabbedExample nodes={tonesNodes} title={fr("Tonalités", "Tones")} />
  </section>
  ...
  <section class="docs-section">
    ...
    <TabbedExample nodes={actionsNodes} title={fr("Avec actions", "With actions")} />
  </section>
  ...
  <section class="docs-section">
    ...
    <TabbedExample nodes={dismissNodes} title={fr("Avec fermeture", "With dismiss")} />
  </section>
