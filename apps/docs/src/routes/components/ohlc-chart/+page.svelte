<script lang="ts">
  ...
  const copy = {
    ...
    en: {
      ...
      badge: "Documented",
      intro:
        "OHLCChart visualises OHLC (Open / High / Low / Close) data as financial bars: a vertical low-to-high bar, a left tick at the open level and a right tick at the close level. Bullish periods are green, bearish ones red.",
      ...
      a11yNote:
        "`label` feeds the `role=\"img\"` container. Each bar exposes its O/H/L/C values in an accessible list outside the SVG.",
...
<div class="docs-page">
  ...
  <section class="docs-section">
    ...
    <table class="docs-table">
      ...
      <tbody>
        <tr><td><code>data</code></td><td><code>OHLCChartDatum[]</code></td><td><em>{text().required}</em></td></tr>
    ...
    <p class="docs-demo-note">
      <code>OHLCChartDatum</code> = <code>{`{ label: string; open: number; high: number; low: number; close: number }`}</code>
    </p>
  </section>
