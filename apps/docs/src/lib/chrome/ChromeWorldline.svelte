<!--
  Chrome documentaire Worldline (worldline.com — paiements / fintech europeen).
  Forme fidele a l'identite Worldline : clair, bleu institutionnel, accent cyan.
  - Header : bandeau BLANC, logo OFFICIEL Worldline (wordmark gris + accent cyan) a
    gauche, nav au centre, recherche bleue a droite
  - Onglet actif = SOULIGNE Worldline Blue #0066a1 + texte bleu ; item sidebar actif
    = lisere bleu + fond bleu tres clair
  - Couleurs MESUREES (theme-worldline) : Worldline Blue #0066a1 (primaire, survol
    #00517f), encre #0c2233, gris #2f3a42, surface #f4f7f9, bordure #e6e6e6 (gris officiel)
  - Logo OFFICIEL Worldline (vecteur Wikimedia) via <img src="/chrome/worldline/logo.svg">
  - Typo : sans corporate (nom public non confirme) → repli systeme (system-ui,
    'Segoe UI') sans chargement reseau
-->
<script lang="ts">
...
<div class="wl-shell">
  ...
  <div class="wl-header-wrap">
    <header class="wl-header" aria-label="Worldline">
      <div class="wl-header__inner">
        <!-- Gauche : logo officiel Worldline -->
        <div class="wl-header__brand">
          <a href="/" class="wl-header__brand-link" aria-label="Accueil : Worldline Design System">
            <img
              src="/chrome/worldline/logo.svg"
              alt="Worldline"
              class="wl-logo"
              width="142"
              height="26"
  ...
  <!-- ── FOOTER Worldline ── -->
  <footer class="wl-footer" aria-label="Pied de page Worldline">
...
<style>
  /* ── Variables Worldline ── */
  .wl-shell {
    --wl-primary: #0066a1; /* Worldline Blue (primaire / action) */
    --wl-primary-hover: #00517f; /* bleu plus profond (survol) */
    --wl-primary-light: #e5eff5; /* bleu tres clair (item actif sidebar) */
    --wl-ink: #0c2233; /* corps / titres (dark navy) */
    --wl-secondary: #2f3a42; /* gris secondaire */
    --wl-subtle: #f4f7f9; /* surface */
    --wl-border: #e6e6e6; /* bordure subtile (gris officiel Worldline) */
    --wl-border-strong: #c2c8cd; /* bordure champ */
    --wl-focus: #0066a1; /* anneau focus bleu */
    --wl-white: #fff;
  ...
  /* Logo officiel Worldline (ratio préservé). */
  .wl-logo {
    display: block;
    width: auto;
    height: 26px;
  ...
  /* Overrides switchers dans header Worldline. */
  ...
  .wl-header__tools-links :global(.docs-header-control[aria-expanded="true"]) {
    background: var(--wl-subtle);
    border-color: var(--wl-primary);
    color: var(--wl-primary);
    box-shadow: none;
  }

  /* Recherche Worldline : bouton loupe compact. */
  .wl-search__btn {
    align-items: center;
