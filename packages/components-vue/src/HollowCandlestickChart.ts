...
export const HollowCandlestickChart = defineComponent({
  name: "HollowCandlestickChart",
  props: {
    data: { type: Array as () => HollowCandlestickChartDatum[], default: () => [] },
  ...
  setup(props, { attrs }) {
    ...
    return () => {
      ...
      // Filter invalid candles BEFORE domain
      ...
      const candles: Candle[] = validData.map((d, i) => {
        ...
        // Colour: close vs PREVIOUS close (first candle: close ≥ open)
        const prevClose = i > 0 ? validData[i - 1].close : d.open;
        const up = d.close >= prevClose;
        // Fill: hollow if bullish (close ≥ open), filled otherwise
        ...
        return {
          datum: d, index: i, up, hollow, centerX,
      ...
      for (const tick of ticks) {
        ...
        svgChildren.push(h("line", { key: `g${tick}`, class: "st-hollowCandlestickChart__grid", x1: MARGIN.left, x2: width - MARGIN.right, y1: ty, y2: ty }));
        svgChildren.push(h("text", { key: `t${tick}`, class: "st-hollowCandlestickChart__tickLabel", x: MARGIN.left - 6, y: ty, "text-anchor": "end", "dominant-baseline": "middle" }, formatTick(tick)));
      }

      // axes
      svgChildren.push(h("line", { class: "st-hollowCandlestickChart__axis", x1: MARGIN.left, x2: MARGIN.left, y1: MARGIN.top, y2: height - MARGIN.bottom }));
      svgChildren.push(h("line", { class: "st-hollowCandlestickChart__axis", x1: MARGIN.left, x2: width - MARGIN.right, y1: height - MARGIN.bottom, y2: height - MARGIN.bottom }));
      ...
      for (const c of candles) {
        ...
        svgChildren.push(h("line", {
          key: `wick${i}-${c.datum.label}`,
          class: classNames(
            "st-hollowCandlestickChart__wick",
            `st-hollowCandlestickChart__wick--${c.up ? "up" : "down"}`,
          ),
        ...
        svgChildren.push(h("rect", {
          ...
          class: classNames(
            ...
            isDim ? "st-hollowCandlestickChart__candle--dim" : undefined,
          ),
          x: c.bodyX, y: c.bodyY,
      ...
      if (hoveredCandle) {
        children.push(h("div", {
          ...
          h("span", { class: "st-hollowCandlestickChart__tooltipLabel" }, hoveredCandle.datum.label),
          h("span", { class: "st-hollowCandlestickChart__tooltipValue" }, `O ${hoveredCandle.datum.open} H ${hoveredCandle.datum.high} L ${hoveredCandle.datum.low} C ${hoveredCandle.datum.close}`),
        ]));
      }

      return h("div", { ...attrs, class: classNames("st-hollowCandlestickChart", props.class) }, children);
    };
  },
});
