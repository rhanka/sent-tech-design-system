...
describe("HollowCandlestickChart", () => {
  it("renders one candle (body) per period", () => {
    const wrapper = mount(HollowCandlestickChart, { props: { label: "Cours", data: week } });
    const el = wrapper.element as HTMLElement;
    expect(el.querySelectorAll(".st-hollowCandlestickChart__candle").length).toBe(week.length);
    expect(el.querySelectorAll(".st-hollowCandlestickChart__wick").length).toBe(week.length);
  });

  it("hollows the body when close ≥ open and fills it when close < open", () => {
    const wrapper = mount(HollowCandlestickChart, { props: { label: "Cours", data: week } });
    const candles = (wrapper.element as HTMLElement).querySelectorAll(".st-hollowCandlestickChart__candle");
  ...
  it("colours the first candle up when close ≥ open", () => {
    const wrapper = mount(HollowCandlestickChart, { props: { label: "Cours", data: week } });
    const wicks = (wrapper.element as HTMLElement).querySelectorAll(".st-hollowCandlestickChart__wick");
  ...
  it("drops non-finite periods before rendering", () => {
    const wrapper = mount(HollowCandlestickChart, {
      props: {
        label: "Filtré",
        data: [
    ...
    const el = wrapper.element as HTMLElement;
    expect(el.querySelectorAll(".st-hollowCandlestickChart__candle").length).toBe(1);
  });
});
