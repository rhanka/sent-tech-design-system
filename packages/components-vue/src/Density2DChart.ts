  ...
  setup(props, { attrs }) {
    ...
    return () => {
      ...
      const binCount = Math.max(
        1,
        Math.min(40, Math.floor(Number.isFinite(props.bins) ? props.bins : 12)),
      );
        ...
        svgChildren.push(
          h("rect", {
            ...
            class: classNames(
              ...
              isDim ? "st-density2DChart__cell--dim" : undefined,
            ),
            x: cell.x,
            y: cell.y,
            width: cell.width,
      ...
      const hoveredCell =
        hoveredKey.value !== null ? binCells.find((b) => b.key === hoveredKey.value) ?? null : null;
      ...
      if (hasLegend) {
        children.push(
          h("div", { class: "st-density2DChart__legend", "aria-hidden": "true" }, [
            h("span", { class: "st-density2DChart__legendText" }, "Low"),
            h(
              "span",
              { class: "st-density2DChart__legendRamp" },
        ...
        children.push(
          h(
            ...
            [
              h(
                "span",
                { class: "st-density2DChart__tooltipLabel" },
                `[${formatTick(hoveredCell.x0)}–${formatTick(hoveredCell.x1)}] × [${formatTick(hoveredCell.y0)}–${formatTick(hoveredCell.y1)}]`,
              ),
