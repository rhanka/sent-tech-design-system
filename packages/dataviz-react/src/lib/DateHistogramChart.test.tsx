...
describe('DateHistogramChart (react)', () => {
  it('renders date buckets through DS BarChart and stores a scoped range selection', () => {
    ...
    const { container } = render(
      <DateHistogramChart store={store} viewId="events" date="ts" interval="day" label="Events by day" formatLabel={formatLabel} />,
    );

    expect(screen.getByRole('img', { name: 'Events by day' })).toBeTruthy();
    expect(container.querySelectorAll('.st-dateHistogramChart .st-barChart__bar')).toHaveLength(2);

    act(() => {
      screen.getByRole('button', { name: /^day 1:/ }).click();
    });
