...
describe('SelectionLegend (react)', () => {
  it('renders nothing while there are no selections', () => {
    const { container } = render(<SelectionLegend store={newStore()} />);
  ...
  it('renders one chip per view with a count and a friendly label', () => {
    ...
    render(<SelectionLegend store={store} labels={{ byCountry: 'Carte' }} />);
  ...
  it('falls back to the view id when no label is supplied', () => {
    const store = newStore();
    store.toggleSelection('byCountry', 'FR');
    render(<SelectionLegend store={store} />);
  ...
  it('appears reactively when a selection is made after mount', () => {
    const store = newStore();
    const { container } = render(<SelectionLegend store={store} />);
    expect(container.querySelector('[role="group"]')).toBeNull();
    act(() => {
      store.toggleSelection('byCountry', 'FR');
    });
