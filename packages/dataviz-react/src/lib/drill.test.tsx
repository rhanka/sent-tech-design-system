...
describe('DrillChart (react)', () => {
  it('renders the top level (bar) with the first hierarchy level', () => {
    render(<DrillChart store={newStore()} kind="bar" {...base} />);
  ...
  it('renders the top level (donut) with a drill-control per top-level key', () => {
    render(<DrillChart store={newStore()} kind="donut" {...base} />);
    expect(screen.getByRole('button', { name: /^Nord:/ })).toBeTruthy();
    expect(screen.getByRole('button', { name: /^Sud:/ })).toBeTruthy();
  });

  it('renders the top level (treemap) with a drill-control per top-level key', () => {
    render(<DrillChart store={newStore()} kind="treemap" {...base} />);
    expect(screen.getByRole('button', { name: /^Nord:/ })).toBeTruthy();
    expect(screen.getByRole('button', { name: /^Sud:/ })).toBeTruthy();
  });
});
