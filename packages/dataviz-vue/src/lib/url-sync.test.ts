...
import { createDashboardStore, type DataModel, type DashboardState, type DashboardStore } from '@sentropic/dataviz-core';
import {
  stateToQuery,
  queryToState,
  applyStateToStore,
...
const Harness = defineComponent({
  props: { store: { type: Object as PropType<DashboardStore>, required: true } },
  setup(props) {
    useUrlSync(props.store);
    return () => h('div');
  },
});
...
describe('useUrlSync (live binding)', () => {
  ...
  it('tears down its subscription on unmount', () => {
    const store = newStore();
    const wrapper = mount(Harness, { props: { store } });
    wrapper.unmount();
    window.history.replaceState(null, '', '/');
    // After unmount, store changes must not write to the URL.
    store.setFilter('country', { kind: 'include', values: ['FR'] });
