...
describe('TopNFilter (vue)', () => {
  it('applies an include filter of the top-N values by measure on mount', () => {
    const store = newStore();
    mount(TopNFilter, { props: { store, dimension: 'country', measure: 'sales', defaultN: 2 } });
    expect(values(store.getState().filters.country)).toEqual(new Set(['US', 'FR']));
  });

  it('honours a different N', () => {
    const store = newStore();
    mount(TopNFilter, { props: { store, dimension: 'country', measure: 'sales', defaultN: 1 } });
    expect(values(store.getState().filters.country)).toEqual(new Set(['US']));
  });

  it('renders a labelled number input', () => {
    const w = mount(TopNFilter, {
      props: { store: newStore(), dimension: 'country', measure: 'sales', label: 'Top N pays' },
    });
    expect(w.text()).toContain('Top N pays');
  });
});
