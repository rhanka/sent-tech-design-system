...
export type AreaRangeChartProps = Omit<React.HTMLAttributes<HTMLDivElement>, "className"> & {
...
export function AreaRangeChart({
  ...
  const gradientId = `st-arearangechart-gradient-${reactId.replace(/:/g, "")}`;
  ...
  // Band: high line (left→right) then low line reversed (right→left) + Z.
  const areaPath = (() => {
  ...
  return (
    <div {...rest} className={classNames("st-areaRangeChart", `st-areaRangeChart--${tone}`, className)}>
      <div
        className="st-areaRangeChart__visual"
        role="img"
        aria-label={label}
        ...
        >
          ...
          {gridLines.map((g) => (
            <React.Fragment key={g.value}>
              <line className="st-areaRangeChart__grid" x1={MARGIN.left} x2={width - MARGIN.right} y1={g.y} y2={g.y} />
              <text
                className="st-areaRangeChart__tickLabel"
                x={MARGIN.left - 6}
                y={g.y}
                textAnchor="end"
          ...
          <line className="st-areaRangeChart__axis" x1={MARGIN.left} x2={MARGIN.left} y1={MARGIN.top} y2={height - MARGIN.bottom} />
          <line
            className="st-areaRangeChart__axis"
            x1={MARGIN.left}
          ...
          {areaPath ? <path className="st-areaRangeChart__area" d={areaPath} fill={`url(#${gradientId})`} /> : null}
          {highPath ? (
            <path
              className="st-areaRangeChart__line st-areaRangeChart__line--high"
              d={highPath}
              fill="none"
              strokeWidth="2"
              strokeLinecap="round"
          ...
          {lowPath ? (
            <path
              className="st-areaRangeChart__line st-areaRangeChart__line--low"
              d={lowPath}
              fill="none"
              strokeWidth="2"
              strokeLinecap="round"
          ...
          {points.map((p) => (
            <React.Fragment key={p.index}>
              <circle className="st-areaRangeChart__dot" cx={p.x} cy={p.yHigh} r="3.5" data-chart-index={p.index} />
              <circle className="st-areaRangeChart__dot" cx={p.x} cy={p.yLow} r="3.5" data-chart-index={p.index} />
            </React.Fragment>
          ))}
        </svg>
      </div>
      ...
      {hoveredPoint ? (
        <div
          className="st-areaRangeChart__tooltip"
          role="presentation"
          style={{ left: `${(hoveredPoint.x / width) * 100}%`, top: `${(hoveredPoint.yHigh / height) * 100}%` }}
        >
          <span className="st-areaRangeChart__tooltipLabel">{hoveredPoint.datum.x}</span>
          <span className="st-areaRangeChart__tooltipValue">{`${hoveredPoint.range.lo} – ${hoveredPoint.range.hi}`}</span>
        </div>
      ) : null}
    </div>
  );
