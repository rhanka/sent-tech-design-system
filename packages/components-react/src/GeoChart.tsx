...
export type GeoChartProps = Omit<React.HTMLAttributes<HTMLDivElement>, "className"> & {
...
export function GeoChart({
  ...
  const mapBounds = validBounds(bounds) ?? fitBounds(layers ?? []);
  ...
  const rendered = (layers ?? []).map((layer, layerIndex) => {
    ...
    switch (layer.type) {
      case "geojson": {
        ...
        return (
          <g key={key} className="st-geoMap__layer st-geoMap__layer--geojson">
      ...
      case "choropleth": {
        ...
        return (
          <g key={key} className="st-geoMap__layer st-geoMap__layer--choropleth">
            {regions.map((region) =>
              region.mix === null ? (
                <path key={region.key} className="st-geoMap__region st-geoMap__region--empty" d={region.d} fillRule="evenodd" />
              ) : (
                <path
                  key={region.key}
                  className={classNames("st-geoMap__region", `st-geoMap__region--${tone}`)}
                  d={region.d}
                  fillRule="evenodd"
                  style={{ "--st-geoMap-mix": `${region.mix}%` } as React.CSSProperties}
                />
              ),
            )}
          </g>
      ...
      case "points": {
        ...
        return (
          <g key={key} className="st-geoMap__layer st-geoMap__layer--points">
            {marks.map((mark) => (
              <circle
                key={mark.key}
                className={classNames("st-geoMap__point", `st-geoMap__point--${mark.tone}`)}
                cx={mark.cx}
                cy={mark.cy}
                r={mark.r}
              />
      ...
      case "density": {
        ...
        return (
          <g key={key} className="st-geoMap__layer st-geoMap__layer--density">
            {marks.map((mark) => (
              <circle
                key={mark.key}
                className={classNames("st-geoMap__density", `st-geoMap__density--${tone}`)}
                cx={mark.cx}
                cy={mark.cy}
                r={mark.r}
                style={{ "--st-geoMap-mix": `${mark.mix}%` } as React.CSSProperties}
              />
            ))}
          </g>
        );
      ...
      case "flow": {
        ...
        return (
          <g key={key} className="st-geoMap__layer st-geoMap__layer--flow">
            {marks.map((mark) => (
              <path
                key={mark.key}
                className={classNames("st-geoMap__flow", `st-geoMap__flow--${tone}`)}
                d={mark.d}
      ...
      case "hexbin": {
        ...
        return (
          <g key={key} className="st-geoMap__layer st-geoMap__layer--hexbin">
            {marks.map((mark) => (
              <polygon
                key={mark.key}
                className={classNames("st-geoMap__hexbin", `st-geoMap__hexbin--${mark.tone}`)}
                points={mark.points}
                style={{ "--st-geoMap-mix": `${mark.mix}%` } as React.CSSProperties}
              />
            ))}
      ...
      case "cluster": {
        ...
        return (
          <g key={key} className="st-geoMap__layer st-geoMap__layer--cluster">
            {marks.map((mark) => (
              <g key={mark.key} className={classNames("st-geoMap__cluster", `st-geoMap__cluster--${mark.tone}`)}>
                <circle className="st-geoMap__clusterDot" cx={mark.cx} cy={mark.cy} r={mark.r} />
                <circle className="st-geoMap__clusterRing" cx={mark.cx} cy={mark.cy} r={mark.r + 3} />
              </g>
            ))}
          </g>
        );
  ...
  return (
    <div {...rest} className={classNames("st-geoMap", className)}>
      <div className="st-geoMap__visual" role="img" aria-label={label}>
        <svg
          ...
          aria-hidden="true"
        >
          <rect className="st-geoMap__frame" x="0.5" y="0.5" width={width - 1} height={height - 1} rx="4" />
          {rendered}
        </svg>
      </div>
