...
const arrows = (container: HTMLElement) => Array.from(container.querySelectorAll(".st-vectorFieldChart__arrow"));
...
describe("VectorFieldChart", () => {
  it("renders an img role and one arrow per datum", () => {
    const { container } = render(<VectorFieldChart data={field} label="Vent" />);
    expect(container.querySelector('[role="img"]')).toBeTruthy();
    expect(arrows(container).length).toBe(4);
  });

  it("draws a shaft and two head segments per arrow", () => {
    const { container } = render(<VectorFieldChart data={field} label="V" />);
  ...
  it("lists every datum in the accessible data list", () => {
    const { container } = render(<VectorFieldChart data={[{ x: 2, y: 3, length: 5, direction: 45 }]} label="V" />);
  ...
  it("merges a custom class onto the root", () => {
    const { container } = render(<VectorFieldChart data={field} className="mine" />);
