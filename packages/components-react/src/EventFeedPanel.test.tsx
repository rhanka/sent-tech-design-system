...
const items = (container: HTMLElement) => Array.from(container.querySelectorAll(".st-eventFeedPanel__item"));

describe("EventFeedPanel", () => {
  it("renders a feed with one article per event", () => {
    const { container } = render(<EventFeedPanel data={events} label="Activité" />);
  ...
  it("sorts events by timestamp descending (newest first)", () => {
    const { container } = render(<EventFeedPanel data={events} label="A" />);
  ...
  it("falls back to the neutral tone for unknown severities", () => {
    const { container } = render(
      <EventFeedPanel data={[{ at: 1, type: "x", severity: "debug", message: "noise" }]} label="A" />,
    );
  ...
  it("constrains the scroll height when maxHeight is set", () => {
    const { container } = render(<EventFeedPanel data={events} maxHeight={180} />);
  ...
  it("accepts height as an alias of maxHeight", () => {
    const { container } = render(<EventFeedPanel data={events} height={120} />);
    const list = container.querySelector(".st-eventFeedPanel__list") as HTMLElement;
    expect(list.style.maxHeight).toBe("120px");
  });

  it("merges a custom class onto the root", () => {
    const { container } = render(<EventFeedPanel data={events} className="mine" />);
