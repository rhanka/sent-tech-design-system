...
export type OrganizationChartProps = Omit<React.HTMLAttributes<HTMLDivElement>, "className"> & {
...
function buildLayout(data: OrganizationChartNode[], width: number, height: number): Layout {
  ...
  for (const b of boxes) {
    ...
    if (!parent) continue;
    links.push({ id: b.id, x1: parent.x, y1: parent.y + BOX_H, x2: b.x, y2: b.y });
  }
...
export function OrganizationChart({
  ...
  const { boxes, links, boxW } = buildLayout(data, width, height);

  const fontSize = boxW < 56 ? 9 : boxW < 88 ? 10 : 12;
  const maxChars = Math.max(2, Math.floor((boxW - 10) / (fontSize * 0.58)));

  const dataValueItems = boxes.map((b) => {
    const parent = b.parentId != null ? boxes.find((p) => p.id === b.parentId) : undefined;
    return parent ? `${b.label} (${b.id}) → ${parent.label}` : `${b.label} (${b.id})`;
  });

  return (
    <div {...rest} className={classNames("st-organizationChart", className)}>
      <div className="st-organizationChart__visual" role="img" aria-label={label}>
        <svg
        ...
        >
          {links.map((link) => {
            ...
            return (
              <path
                key={link.id}
                className="st-organizationChart__link"
          ...
          {boxes.map((box) => (
            <g key={box.id} className="st-organizationChart__node">
              <rect
                className={classNames(
                  "st-organizationChart__box",
                  `st-organizationChart__box--${box.tone}`,
                )}
                x={box.x - boxW / 2}
                y={box.y}
                width={boxW}
              ...
              />
              <text
                className="st-organizationChart__label"
                x={box.x}
                y={box.y + BOX_H / 2}
                textAnchor="middle"
