...
describe("AppChrome — navigation", () => {
  ...
  it("filters role-gated items and renders disabled nav without href", () => {
    const { container } = render(<AppChrome nav={[...nav, { label: "Admin", href: "/admin", role: "admin" }, { label: "Soon", href: "/soon", disabled: true }]} userRoles={["member"]} />);
    const links = Array.from(container.querySelectorAll<HTMLAnchorElement>(".st-appChrome__navLink"));
    expect(links.map((a) => a.textContent)).toEqual(["Vues", "Données", "Réglages", "Soon"]);
...
describe("AppChrome — extraSelectors", () => {
  it("renders extraSelectors content in the utility nav", () => {
    const { container } = render(<AppChrome extraSelectors={<span>extra-ctrl</span>} />);
