...
const cells = (container: HTMLElement) => Array.from(container.querySelectorAll(".st-contourChart__cell"));
...
describe("ContourChart", () => {
  it("renders an img role and one cell per datum", () => {
    const { container } = render(<ContourChart data={grid} label="Relief" />);
    expect(container.querySelector('[role="img"]')).toBeTruthy();
    expect(cells(container).length).toBe(4);
  });

  it("colours cells by value band (largest value → category8)", () => {
    const { container } = render(<ContourChart data={grid} label="C" />);
  ...
  it("renders a level legend ramp", () => {
    const { container } = render(<ContourChart data={grid} levels={4} label="C" />);
  ...
  it("lists every datum in the accessible data list", () => {
    const { container } = render(<ContourChart data={[{ x: 2, y: 3, value: 5 }]} label="C" />);
  ...
  it("merges a custom class onto the root", () => {
    const { container } = render(<ContourChart data={grid} className="mine" />);
