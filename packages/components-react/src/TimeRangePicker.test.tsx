...
// ---------------------------------------------------------------------------
// TimeRangePicker — component behaviour
// ---------------------------------------------------------------------------
//
// Deviation from the Svelte suite: the React `ContentSwitcher` primitive
// (src/catalog.tsx) renders plain `<button>` options (role="group" wrapper,
// `aria-pressed`) rather than the Svelte version's `role="tablist"` /
// `role="tab"` markup — a pre-existing cross-framework gap in the shared
// primitive (see svelte-api-parity.test.tsx's own ContentSwitcher assertion,
// which already queries `getByRole("button", ...)`). Tab-switch queries below
// use `getByRole("button", ...)` accordingly instead of `getByRole("tab", ...)`.
...
describe("TimeRangePicker — trigger label", () => {
  ...
  it("shows an English relative label when locale='en-US'", () => {
    ...
    const { container } = render(<TimeRangePicker value={value} locale="en-US" />);
  ...
  it("collapses an absolute same-day range on the trigger", () => {
    const from = new Date(2024, 2, 10, 9, 0).getTime();
    const to = new Date(2024, 2, 10, 17, 30).getTime();
    const { container } = render(<TimeRangePicker value={{ mode: "absolute", from, to }} />);
  ...
  it("honours a custom formatRange override", () => {
    const { container } = render(<TimeRangePicker formatRange={() => "CUSTOM LABEL"} />);
    expect(triggerLabelText(container)).toBe("CUSTOM LABEL");
  });
});

describe("TimeRangePicker — open/close mechanics", () => {
  it("opens a role=dialog panel on trigger click and reflects aria-expanded", () => {
    const { container } = render(<TimeRangePicker />);
  ...
  it("Escape closes the panel without emitting and returns focus to the trigger", () => {
  ...
  it("an outside pointerdown closes the panel without emitting", () => {
    const onChange = vi.fn();
    const { container } = render(<TimeRangePicker onChange={onChange} />);
    openPanel(container);
    expect(container.querySelector('[role="dialog"]')).toBeTruthy();
  ...
  it("traps Tab navigation within the panel (Shift+Tab wraps last -> first)", () => {
    const { container } = render(<TimeRangePicker />);
    openPanel(container);
    const dialog = container.querySelector('[role="dialog"]') as HTMLElement;
...
describe("TimeRangePicker — relative presets", () => {
  it("clicking a preset resolves now, emits {mode:'relative',...}, and closes", () => {
    const onChange = vi.fn();
    const { container } = render(<TimeRangePicker onChange={onChange} />);
    openPanel(container);
  ...
  it("re-clicking the already-active preset still refreshes now and closes", () => {
    const onChange = vi.fn();
    const value: TimeRange = { mode: "relative", relative: "1h", from: Date.now() - 3_601_000, to: Date.now() - 1_000 };
    const { container } = render(<TimeRangePicker value={value} onChange={onChange} />);
    openPanel(container);
    const row = container.querySelector('[data-value="1h"]') as HTMLElement;
  ...
  it("does not emit or close when clicking a disabled preset", () => {
    const onChange = vi.fn();
    const now = Date.now();
    const { container } = render(
      <TimeRangePicker presets={["30m"]} max={now - 2 * 86_400_000} onChange={onChange} />
    );
    openPanel(container);
  ...
  it("accepts object-form presets with an explicit label and durationMs override", () => {
    const onChange = vi.fn();
    const { container } = render(
      <TimeRangePicker presets={[{ token: "custom", label: "Custom 45", durationMs: 45 * 60_000 }]} onChange={onChange} />
    );
    openPanel(container);
  ...
  it("honours a custom formatPresetLabel override for preset rows", () => {
    const { container } = render(<TimeRangePicker presets={["1h"]} formatPresetLabel={(token) => `>>${token}<<`} />);
    openPanel(container);
    const row = container.querySelector('[data-value="1h"]');
    expect(row?.textContent?.trim()).toBe(">>1h<<");
  });

  it("uses DEFAULT_TIME_RANGE_PRESETS when `presets` is omitted", () => {
    const { container } = render(<TimeRangePicker />);
    openPanel(container);
...
describe("TimeRangePicker — custom tab: Apply/Cancel", () => {
  it("Apply emits {mode:'absolute'} with no `relative`, from<=to, and closes", () => {
    const onChange = vi.fn();
    const { container, getByRole, getByLabelText } = render(<TimeRangePicker onChange={onChange} locale="en-US" />);
    openPanel(container);
    ...
    fireEvent.input(fromInput, { target: { value: "2024-01-01 10:00" } });
    fireEvent.blur(fromInput);
    fireEvent.input(toInput, { target: { value: "2024-01-02 11:30" } });
  ...
  it("Cancel discards the draft and emits nothing", () => {
    const onChange = vi.fn();
    const { container, getByRole, getByLabelText } = render(<TimeRangePicker onChange={onChange} locale="en-US" />);
    openPanel(container);
    fireEvent.click(getByRole("button", { name: "Custom" }));
  ...
  it("disables Apply and shows an inline error when from > to", () => {
    const { container, getByRole, getByLabelText } = render(<TimeRangePicker locale="en-US" />);
    openPanel(container);
    fireEvent.click(getByRole("button", { name: "Custom" }));
    const fromInput = getByLabelText("From") as HTMLInputElement;
    const toInput = getByLabelText("To") as HTMLInputElement;
    fireEvent.input(fromInput, { target: { value: "2024-01-05 10:00" } });
    fireEvent.blur(fromInput);
    fireEvent.input(toInput, { target: { value: "2024-01-01 10:00" } });
    fireEvent.blur(toInput);
    const applyBtn = getByRole("button", { name: "Apply" }) as HTMLButtonElement;
  ...
  it("disables Apply while the draft is incomplete (half-picked calendar range)", () => {
  ...
  it("renders one Calendar when calendarMonths=1", () => {
    const { container, getByRole } = render(<TimeRangePicker calendarMonths={1} />);
    openPanel(container);
    fireEvent.click(getByRole("button", { name: "Personnalisé" }));
    expect(container.querySelectorAll(".st-calendar").length).toBe(1);
  });

  it("renders two side-by-side Calendars by default (calendarMonths=2)", () => {
...
describe("TimeRangePicker — controlled vs uncontrolled", () => {
  it("uncontrolled: manages its own state and still calls onChange", () => {
    const onChange = vi.fn();
    const { container } = render(<TimeRangePicker onChange={onChange} />);
    openPanel(container);
  ...
  it("controlled: reflects a `value` pushed by the consumer via rerender", () => {
    const onChange = vi.fn();
    const v1: TimeRange = { mode: "absolute", from: new Date(2024, 0, 1, 8, 0).getTime(), to: new Date(2024, 0, 1, 9, 0).getTime() };
    const { container, rerender } = render(<TimeRangePicker value={v1} onChange={onChange} />);
    ...
    rerender(<TimeRangePicker value={v2} onChange={onChange} />);
    expect(triggerLabelText(container)).toBe(formatPresetLabel("1h", "fr-FR"));
  });

  it("controlled: clicking a preset notifies onChange with the resolved value", () => {
    const onChange = vi.fn();
...
describe("TimeRangePicker — min/max clamping", () => {
  it("disables a preset that resolves entirely outside the max bound", () => {
    const now = Date.now();
    const { container } = render(<TimeRangePicker presets={["30m", "24h"]} max={now - 2 * 86_400_000} />);
    openPanel(container);
    const shortRow = container.querySelector('[data-value="30m"]') as HTMLElement;
    expect(shortRow.getAttribute("aria-disabled")).toBe("true");
  });

  it("keeps a preset enabled (clamped) when only part of its window is out of bounds", () => {
    const now = Date.now();
    const { container } = render(<TimeRangePicker presets={["24h"]} min={now - 60_000} />);
    openPanel(container);
...
describe("TimeRangePicker — misc props", () => {
  it("merges a custom class and applies the size modifier", () => {
    const { container } = render(<TimeRangePicker className="mine" size="lg" />);
  ...
  it("renders the field label and wires it into the trigger's accessible name", () => {
    const { container } = render(<TimeRangePicker label="Time window" />);
