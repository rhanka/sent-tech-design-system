...
describe("RibbonChart", () => {
  it("renders one segment per datum across all periods", () => {
    const { container } = render(<RibbonChart label="Rang" data={data} />);
  ...
  it("honours an explicit tone on a category", () => {
    const { container } = render(
      <RibbonChart
        label="Rang"
        data={[
          { category: "Card", period: "Q1", value: 10, tone: "category5" },
          { category: "Card", period: "Q2", value: 8, tone: "category5" },
        ]}
      />,
    );
  ...
  it("summarises each category in the accessible data list", () => {
    const { container } = render(<RibbonChart label="Rang" data={data} />);
    expect(listItems(container)[0]).toBe("Card: Q1 = 42, Q2 = 35");
  });
