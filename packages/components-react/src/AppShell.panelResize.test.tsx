...
describe("AppShell — panelResize default (false)", () => {
  ...
  it("does not set an inline style on the shell root (byte-identical to today)", () => {
    const { container } = render(
      <AppShell variant="workspace" {...panelProps} defaultPanelWidths={{ navigationPanel: 400 }} />,
    );
...
describe("AppShell — panelResize=true", () => {
  it("renders exactly one resize handle per RESIZABLE present panel (navigation/context/utility)", () => {
    const { container } = render(<AppShell variant="workspace" panelResize {...panelProps} />);
  ...
  it("labels each handle 'Resize {panel label}', falling back to the existing aria-label props", () => {
    const { container } = render(<AppShell variant="workspace" panelResize {...panelProps} />);
    expect(
      container.querySelector(".st-appShell__navigationPanel .st-appShell__resizeHandle")?.getAttribute("aria-label"),
    ).toBe("Resize Workspace navigation");
    expect(
      container.querySelector(".st-appShell__contextPanel .st-appShell__resizeHandle")?.getAttribute("aria-label"),
    ).toBe("Resize Context panel");
    expect(
      container.querySelector(".st-appShell__utilityPanel .st-appShell__resizeHandle")?.getAttribute("aria-label"),
    ).toBe("Resize Utility panel");
  });
  ...
  it("honors custom panelMinWidth/panelMaxWidth on every handle", () => {
    const { container } = render(
      <AppShell variant="workspace" panelResize {...panelProps} panelMinWidth={200} panelMaxWidth={300} />,
    );
    const handles = container.querySelectorAll(".st-appShell__resizeHandle");
    expect(handles.length).toBe(3);
    handles.forEach((handle) => {
  ...
  it("renders no handle for a panel that is not provided", () => {
    const { container } = render(
      <AppShell variant="workspace" panelResize navigationPanel={panelProps.navigationPanel} main={panelProps.main} />,
    );
  ...
  it("renders NO handle for utilityPanel when utilitySide='bottom', but keeps handles for the other resizable panels", () => {
    const { container } = render(<AppShell variant="workspace" panelResize utilitySide="bottom" {...panelProps} />);
  ...
  it("does not set a width var for a panel with no explicit/default/controlled width (defers to the token default)", () => {
    const { container } = render(
      <AppShell variant="workspace" panelResize {...panelProps} defaultPanelWidths={{ navigationPanel: 300 }} />,
    );
    const root = container.querySelector(".st-appShell") as HTMLElement;
    expect(root.style.getPropertyValue("--st-appShell-navigation-width")).toBe("300px");
...
describe("AppShell — panelResize keyboard interaction (uncontrolled)", () => {
  ...
  it("clamps at the upper bound: repeated ArrowRight never exceeds panelMaxWidth", () => {
    ...
    const handle = container.querySelector(".st-appShell__navigationPanel .st-appShell__resizeHandle") as HTMLElement;
    fireEvent.keyDown(handle, { key: "ArrowRight" }); // 495 + 16 = 511 -> clamped to 500
    expect(onPanelResize).toHaveBeenCalledWith("navigationPanel", 500);
    fireEvent.keyDown(handle, { key: "ArrowRight" }); // stays clamped, still fires
  ...
  it("clamps at the lower bound: repeated ArrowLeft never goes below panelMinWidth", () => {
    ...
    const handle = container.querySelector(".st-appShell__navigationPanel .st-appShell__resizeHandle") as HTMLElement;
    fireEvent.keyDown(handle, { key: "ArrowLeft" }); // 190 - 16 = 174 -> clamped to 180
  ...
  it("ignores keys other than ArrowLeft/ArrowRight/Home/End", () => {
    const onPanelResize = vi.fn();
    const { container } = render(
      <AppShell variant="workspace" panelResize {...panelProps} onPanelResize={onPanelResize} />,
    );
    const handle = container.querySelector(".st-appShell__navigationPanel .st-appShell__resizeHandle") as HTMLElement;
...
describe("AppShell — panelResize controlled mode (panelWidths)", () => {
  it("never mutates its own state: the rendered width stays exactly what panelWidths says, even after a commit-triggering interaction", () => {
    const onPanelResize = vi.fn();
    const { container } = render(
      <AppShell
        variant="workspace"
  ...
  it("re-renders from a new panelWidths prop (the consumer driving the loop back)", () => {
    const { container, rerender } = render(
      <AppShell variant="workspace" panelResize {...panelProps} panelWidths={{ navigationPanel: 250 }} />,
    );
    const root = container.querySelector(".st-appShell") as HTMLElement;
    expect(root.style.getPropertyValue("--st-appShell-navigation-width")).toBe("250px");
    rerender(<AppShell variant="workspace" panelResize {...panelProps} panelWidths={{ navigationPanel: 400 }} />);
