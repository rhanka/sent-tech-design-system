...
describe("RoseChart parity with Svelte", () => {
  it("renders one equal-angle sector per datum with category tones and SR values", () => {
    ...
    expect(container.querySelector('[role="img"]')?.getAttribute("aria-label")).toBe("Activity rose");
  ...
  it("scales radius by sqrt(value/max) so area is proportional to value", () => {
    const { container } = render(<RoseChart label="Rose scale" width={200} height={200} data={[{ label: "Full", value: 100 }]} />);
  ...
  it("renders empty data without crashing", () => {
    const { container } = render(<RoseChart label="Empty rose" data={[]} />);
    expect(container.querySelector('[role="img"]')?.getAttribute("aria-label")).toBe("Empty rose");
    expect(container.querySelectorAll(".st-roseChart__sector")).toHaveLength(0);
  });

  it("shows a tooltip on pointer move over a sector", () => {
    const { container } = render(
      <RoseChart
        label="Rose hover"
        data={[
    ...
    fireEvent.pointerMove(container.querySelector('.st-roseChart__sector[data-chart-index="1"]') as Element);
...
describe("ViolinChart parity with Svelte", () => {
  it("renders one mirrored shape per category with quartile overlays and SR summaries", () => {
    ...
    expect(container.querySelector('[role="img"]')?.getAttribute("aria-label")).toBe("Latency violins");
  ...
  it("draws a closed mirror path", () => {
    const { container } = render(<ViolinChart label="Violin symmetry" width={200} height={200} data={[{ label: "One", values: [1, 2, 2, 3] }]} />);
  ...
  it("hides quartile overlays when quartiles is false", () => {
    const { container } = render(<ViolinChart label="Violin no quartiles" quartiles={false} data={[{ label: "A", values: [1, 2, 3, 4] }]} />);
    expect(container.querySelectorAll(".st-violinChart__shape")).toHaveLength(1);
  ...
  it("renders empty data without crashing", () => {
    const { container } = render(<ViolinChart label="Empty violin" data={[]} />);
    expect(container.querySelector('[role="img"]')?.getAttribute("aria-label")).toBe("Empty violin");
