...
describe("ErrorBarChart", () => {
  it("renders a whisker line per category", () => {
    const { container } = render(<ErrorBarChart label="Mesures" data={data} />);
  ...
  it("normalizes inverted low/high and clamps value into bounds", () => {
    const { container } = render(<ErrorBarChart label="Inv" data={[{ category: "X", value: 15, low: 10, high: 2 }]} />);
  ...
  it("applies the tone class to whisker, caps and marker", () => {
    const { container } = render(<ErrorBarChart label="Tone" data={data} tone="category3" />);
  ...
  it("renders a legend with the chart label", () => {
    const { getAllByText } = render(<ErrorBarChart label="Croissance" data={data} />);
