...
describe("TileMapChart", () => {
  it("renders one tile per datum with role=img", () => {
    const { container } = render(<TileMapChart label="Régions" data={data} />);
  ...
  it("colors the lowest value with category1 and the highest with category8", () => {
    const { container } = render(<TileMapChart label="Échelle" data={data} />);
  ...
  it("renders a gradient legend", () => {
    const { container } = render(<TileMapChart label="Légende" data={data} />);
  ...
  it("renders empty without crashing for an empty dataset", () => {
    const { container } = render(<TileMapChart label="Empty" data={[]} />);
  ...
  it("shows, updates, and hides tooltip on interactions", () => {
    const { container } = render(<TileMapChart label="Interactive" data={data} />);

    expect(screen.queryByRole("presentation")).toBeNull();

    const tiles = container.querySelectorAll(".st-tileMapChart__tile");
    fireEvent.pointerMove(tiles[2]);

    expect(screen.getByRole("presentation")).toBeTruthy();
    expect(container.querySelector(".st-tileMapChart__tooltipLabel")?.textContent).toBe("ON");

    fireEvent.pointerLeave(container.querySelector(".st-tileMapChart__visual") as Element);
    expect(screen.queryByRole("presentation")).toBeNull();
  });
});
