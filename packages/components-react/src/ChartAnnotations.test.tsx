...
describe("LineChart annotations", () => {
  it("renders nothing extra by default (additive)", () => {
    const { container } = render(<LineChart label="Plain" data={lineData} />);
    expect(container.querySelector(".st-lineChart__annotations")).toBeNull();
  ...
  it("renders the 5 kinds and appends a11y items", () => {
    ...
    const { container } = render(<LineChart label="Ann" data={lineData} annotations={annotations} />);
    expect(container.querySelector(".st-lineChart__annotationRegion")).not.toBeNull();
    expect(container.querySelector(".st-lineChart__annotationLine")).not.toBeNull();
    expect(container.querySelector(".st-lineChart__annotationPoint")).not.toBeNull();
    expect(container.querySelector(".st-lineChart__annotationText")).not.toBeNull();
    expect(container.querySelector(".st-lineChart__annotationShape")).not.toBeNull();
  ...
  it("draws regions BEHIND and other annotations ABOVE the series", () => {
    ...
    const { container } = render(<LineChart label="Order" data={lineData} annotations={annotations} />);
  ...
  it("ignores an out-of-domain annotation (never escapes the plot)", () => {
    const { container } = render(
      <LineChart label="Out" data={lineData} annotations={[{ kind: "point", x: 99, y: 99 }]} />,
    );
    expect(container.querySelector(".st-lineChart__annotationPoint")).toBeNull();
  });
});
...
describe("AreaChart annotations", () => {
  it("renders an annotation point + a11y item", () => {
    const { container } = render(
      <AreaChart label="Area" data={[1, 4, 2, 6]} annotations={[{ kind: "point", x: 2, y: 2, label: "Pic" }]} />,
    );
    expect(container.querySelector(".st-areaChart__annotationPoint")).not.toBeNull();
    expect(listItems(container)).toContain("Annotation: Pic");
  });
});
...
describe("BarChart annotations", () => {
  ...
  it("anchors a categorical point on the matching bar", () => {
    const { container } = render(
      <BarChart label="Bars" data={barData} annotations={[{ kind: "point", x: "B", y: 30, label: "Top" }]} />,
    );
    expect(container.querySelector(".st-barChart__annotationPoint")).not.toBeNull();
    expect(listItems(container)).toContain("Annotation: Top");
  });

  it("ignores a point whose category does not match any bar", () => {
    const { container } = render(
      <BarChart label="Bars" data={barData} annotations={[{ kind: "point", x: "Z", y: 10 }]} />,
    );
    expect(container.querySelector(".st-barChart__annotationPoint")).toBeNull();
  });

  it("draws a value-axis line across the plot", () => {
    const { container } = render(
      <BarChart label="Bars" data={barData} annotations={[{ kind: "line", axis: "y", value: 25, label: "Cible" }]} />,
    );
    expect(container.querySelector(".st-barChart__annotationLine")).not.toBeNull();
  });
});
