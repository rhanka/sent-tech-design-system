...
describe("CandlestickChart", () => {
  it("renders one candle (body + wick) per period", () => {
    const { container } = render(<CandlestickChart label="Cours" data={week} />);
...
describe("CandlestickChart — annotations / dataLabels / crosshair / keyboard nav", () => {
  ...
  it("renders a support line, a price region and a point annotation", () => {
    ...
    const { container } = render(
      <CandlestickChart label="Cours" data={week} annotations={annotations} />,
    );
  ...
  it("renders one close data label per candle (aria-hidden)", () => {
    const { container } = render(<CandlestickChart label="Cours" data={week} dataLabels />);
  ...
  it("draws a controlled crosshair at the candle matching hoverKey", () => {
    const { container, rerender } = render(
      <CandlestickChart label="Cours" data={week} hoverKey={null} />,
    );
    expect(container.querySelector(".st-candlestickChart__crosshairLine")).toBeNull();
    rerender(<CandlestickChart label="Cours" data={week} hoverKey="Mer" />);
  ...
  it("enables a roving keyboard overlay announcing date + O/H/L/C", () => {
    ...
    const { container } = render(
      <CandlestickChart label="Cours" data={week} onSelectKey={onSelectKey} />,
    );
    const datums = Array.from(
      container.querySelectorAll<SVGRectElement>(".st-candlestickChart__navDatum"),
    );
    ...
    fireEvent.keyDown(
      Array.from(container.querySelectorAll<SVGRectElement>(".st-candlestickChart__navDatum"))[4],
      { key: " " },
    );
