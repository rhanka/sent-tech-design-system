...
export class GanttChart {
    ...
    get safeData() {
        return (this.data ?? []).filter((d) => typeof d.task === "string" && d.task.length > 0 && Number.isFinite(d.start) && Number.isFinite(d.end));
    }
    get domainBounds() {
        const vals = [];
        for (const d of this.safeData)
            vals.push(d.start, d.end);
        if (typeof this.marker === "number" && Number.isFinite(this.marker))
            vals.push(this.marker);
        if (vals.length === 0)
    ...
    get markerGeom() {
        if (this.marker == null || !Number.isFinite(this.marker))
            return null;
    ...
    static ɵcmp = i0.ɵɵngDeclareComponent({ minVersion: "17.0.0", version: "21.2.17", type: GanttChart, isStandalone: true, selector: "st-gantt-chart", inputs: { data: "data", label: "label", width: "width", height: "height", marker: "marker", classInput: ["class", "classInput"] }, ngImport: i0, template: `
