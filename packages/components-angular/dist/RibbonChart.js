...
export class RibbonChart {
    static stComponentName = "RibbonChart";
    componentName = "RibbonChart";
    hoveredKey = null;
    data = [];
    label;
    width;
    ...
    get resolvedWidth() { return this.width ?? this.size ?? 520; }
    get heightValue() { return this.height ?? 300; }
    get periodLabelY() { return this.heightValue - MARGIN.bottom + 16; }
    get viewBox() {
    ...
    get validData() {
        return this.data.filter((d) => typeof d.category === "string" &&
            d.category.length > 0 &&
    ...
    get categoryOrder() {
        const seen = [];
        for (const d of this.validData) {
            if (!seen.includes(d.category))
                seen.push(d.category);
        }
        return seen;
    }
    get explicitToneByCategory() {
        const map = new Map();
        for (const d of this.validData) {
            if (d.tone)
                map.set(d.category, d.tone);
        }
        return map;
    }
    toneOf(category) {
    ...
    get columns() {
        if (this.validData.length === 0 || this.periodOrder.length === 0)
            return [];
        const band = this.plotWidth / this.periodOrder.length;
        const barWidth = Math.min(band * 0.5, 72);
        const totals = this.periodOrder.map((p) => this.validData.filter((d) => d.period === p).reduce((s, d) => s + Math.max(d.value, 0), 0));
    ...
    get dataValueItems() {
        return this.categoryOrder.map((category) => `${category}: ${this.periodOrder
            .map((p) => {
    ...
    ribbonClass(ribbon) {
        return classNames("st-ribbonChart__ribbon", `st-ribbonChart__ribbon--${ribbon.tone}`, this.hoveredSegment !== null && this.hoveredSegment.category !== ribbon.category && "st-ribbonChart__ribbon--dim");
    }
    segClass(seg) {
        return classNames("st-ribbonChart__seg", `st-ribbonChart__seg--${seg.tone}`, this.hoveredSegment !== null && this.hoveredSegment.category !== seg.category && "st-ribbonChart__seg--dim");
    }
    segHeight(seg) {
    ...
    static ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.2.17", ngImport: i0, type: RibbonChart, deps: [], target: i0.ɵɵFactoryTarget.Component });
    static ɵcmp = i0.ɵɵngDeclareComponent({ minVersion: "17.0.0", version: "21.2.17", type: RibbonChart, isStandalone: true, selector: "st-ribbon-chart", inputs: { data: "data", label: "label", width: "width", height: "height", size: "size", classInput: ["class", "classInput"] }, ngImport: i0, template: `
...
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.2.17", ngImport: i0, type: RibbonChart, decorators: [{
            type: Component,
            args: [{ selector: "st-ribbon-chart", standalone: true, template: `
    <div [attr.data-st-component]="componentName" [class]="hostClass">
      <div
        class="st-ribbonChart__visual"
        role="img"
