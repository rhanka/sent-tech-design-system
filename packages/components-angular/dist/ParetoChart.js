...
export class ParetoChart {
    ...
    label = "";
    classInput;
    get hostClass() { return classNames("st-paretoChart", this.classInput); }
    ...
    get total() { return this.sortedData.reduce((acc, d) => acc + d.value, 0); }
    get scales() {
    ...
    get entries() {
        ...
        return this.sortedData.map((d, i) => {
            ...
            return { datum: d, tone: (d.tone ?? "category1"), x, y: MARGIN.top + y, width: barWidth, height: Math.max(h, 0.5), cumPercent, cx: MARGIN.left + band * (i + 0.5), cy: MARGIN.top + scaleLinear(cumPercent, 0, 100, plotHeight, 0), index: i };
        });
    }
    get cumulativePath() { return this.entries.map((e, i) => `${i === 0 ? "M" : "L"} ${e.cx} ${e.cy}`).join(" "); }
    get dataValueItems() { return this.entries.map((e) => `${e.datum.label}: ${e.datum.value} (${this.fmtTick(e.cumPercent)}% cumulé)`); }
    get valueAxisTicks() {
    ...
    barClass(e) { return `st-paretoChart__bar st-paretoChart__bar--${e.tone}`; }
    fmtTick(v) {
        if (Math.abs(v) >= 1000)
            return `${(v / 1000).toFixed(v % 1000 === 0 ? 0 : 1)}k`;
        return Number.isInteger(v) ? String(v) : v.toFixed(1);
    }
    tooltipLeft() { const e = this.hoveredIndex !== null ? this.entries[this.hoveredIndex] : null; return e ? `${(e.cx / this.widthValue) * 100}%` : "0%"; }
    tooltipTop() { const e = this.hoveredIndex !== null ? this.entries[this.hoveredIndex] : null; return e ? `${(e.cy / this.heightValue) * 100}%` : "0%"; }
    ...
    static ɵcmp = i0.ɵɵngDeclareComponent({ minVersion: "17.0.0", version: "21.2.17", type: ParetoChart, isStandalone: true, selector: "st-pareto-chart", inputs: { data: "data", width: "width", height: "height", label: "label", classInput: ["class", "classInput"] }, ngImport: i0, template: `
