...
export class AreaRangeChart {
  ...
  readonly gradientId = `st-arearangechart-gradient-${Math.random().toString(36).substring(2, 9)}`;

  private normalize(d: AreaRangeChartDatum): { lo: number; hi: number } | null {
  ...
  private buildPath(pts: { x: number; y: number }[]): string {
    if (this.smooth) return buildSmoothPath(pts);
