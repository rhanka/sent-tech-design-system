...
const CATEGORY_TONES: StateTimelineTone[] = [
  "category1", "category2", "category3", "category4",
  "category5", "category6", "category7", "category8",
];

function scaleLinearLocal(value: number, d0: number, d1: number, r0: number, r1: number): number {
  if (d1 === d0) return r0;
  return r0 + ((value - d0) * (r1 - r0)) / (d1 - d0);
}

function ellipsizeLocal(text: string, maxLen: number): string {
...
export class StateTimelineChart {
  ...
  get safeData(): StateTimelineSeries[] { return this.data ?? []; }
  ...
  get legendItems(): Array<{ state: string; tone: StateTimelineTone }> {
    const seen = new Map<string, StateTimelineTone>();
    for (const series of this.safeData) {
      for (const seg of series.segments) {
        const key = String(seg.state);
        if (!seen.has(key)) {
  ...
  get dataValueItems(): string[] {
    return this.safeData.flatMap((series) =>
      series.segments.map((seg) => `${series.series} / ${seg.state}: ${seg.start} → ${seg.end}`)
    );
  }

  ellipsize(text: string, n: number): string { return ellipsizeLocal(text, n); }
