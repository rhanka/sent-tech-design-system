...
export class ErrorBarChart {
  ...
  readonly MARKER_RADIUS = MARKER_RADIUS;
  readonly CAP_HALF = CAP_HALF;
  ...
  private normalize(d: ErrorBarChartDatum): { lo: number; mid: number; hi: number } | null {
  ...
  get gridLines(): Array<{ value: number; x: number }> {
  ...
  get legendEntries(): Array<{ label: string; shape: "circle"; tone: ErrorBarChartTone }> {
