...
export function keyForX(x: number | string): string {
  return String(x);
}
 ...
 */
export function indexForHoverKey(hoverKey: string | null | undefined, keys: string[]): number {
  if (hoverKey == null) return -1;
...
export function resolveActiveIndex(
  ...
  if (hoverKey !== undefined) return indexForHoverKey(hoverKey, keys);
  return internalIndex == null ? -1 : internalIndex;
}

/** True when the chart is CONTROLLED (the parent supplied `hoverKey`). */
export function isControlled(hoverKey: string | null | undefined): boolean {
