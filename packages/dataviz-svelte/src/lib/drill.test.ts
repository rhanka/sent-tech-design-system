...
describe('drill engine (svelte)', () => {
  ...
  it('resolves the leaf level after drilling: last dimension, cannot drill', () => {
    ...
    // Crossfiltered to Nord: only Lille + Rouen, no Nice.
    expect(lvl.data).toEqual([
      { key: 'Lille', value: 15 },
      { key: 'Rouen', value: 7 },
    ]);
  ...
  it('onDrillSelect at the leaf toggles selection instead of drilling', () => {
    ...
    // Still at the same drill depth — no further drilling happened.
    expect(store.getState().drill.d).toEqual(['city']);
  });
});

describe('DrillChart (svelte)', () => {
  it('renders the top level (bar) with the first hierarchy level', () => {
    const { getByRole } = render(DrillChart, { props: { store: newStore(), kind: 'bar', ...base } });
  ...
  it('renders the top level (donut) with a drill-control per top-level key', () => {
    const { getByRole } = render(DrillChart, { props: { store: newStore(), kind: 'donut', ...base } });
    expect(getByRole('button', { name: /^Nord:/ })).toBeTruthy();
    expect(getByRole('button', { name: /^Sud:/ })).toBeTruthy();
  });

  it('renders the top level (treemap) with a drill-control per top-level key', () => {
    const { getByRole } = render(DrillChart, { props: { store: newStore(), kind: 'treemap', ...base } });
    expect(getByRole('button', { name: /^Nord:/ })).toBeTruthy();
    expect(getByRole('button', { name: /^Sud:/ })).toBeTruthy();
  });
});
