...
} from './RelativeDateFilter.svelte';

const NOW = new Date('2026-06-06T00:00:00Z');
const DAY = 24 * 60 * 60 * 1000;

describe('relativeRangeToSpec', () => {
...
describe('RelativeDateFilter', () => {
  it('renders a labelled select and does not filter for the default "all" preset', async () => {
    const store = createDashboardStore({ model, data: [] as Row[] });
    render(RelativeDateFilter, { props: { store, dimension: 'date', label: 'Période', now: NOW } });
    await tick();
    expect(screen.getByText('Période')).toBeTruthy();
    expect(store.getState().filters.date).toBeUndefined();
  });

  it('applies a trailing-window range filter for a non-"all" initial preset', async () => {
    const store = createDashboardStore({ model, data: [] as Row[] });
    render(RelativeDateFilter, {
      props: { store, dimension: 'date', now: NOW, presets: [{ value: '30d', label: '30 j', days: 30 }] },
    });
    await tick();
