...
import DataImage from "./lib/DataImage.svelte";

describe("Embed (FR2)", () => {
  it("renders a lazy iframe with src + required title inside a ratio container", () => {
    const { container } = render(Embed, { props: { src: "https://example.com", title: "Carte interactive" } });
  ...
  it("defaults to a strict sandbox and 16/9 ratio", () => {
    const { container } = render(Embed, { props: { src: "https://example.com", title: "X" } });
    const frame = container.querySelector("iframe");
    expect(frame?.getAttribute("sandbox")).toBe("");
    expect((container.querySelector<HTMLElement>(".st-aspectRatio")?.style.aspectRatio || "").replace(/\s/g, "")).toBe(
      "16/9",
    );
  });
  ...
  it("honours a custom sandbox, aspectRatio and allow", () => {
    const { container } = render(Embed, {
      props: { src: "s", title: "T", sandbox: "allow-scripts", aspectRatio: "4/3", allow: "fullscreen" },
    });
    const frame = container.querySelector("iframe");
  ...
  it("allows eager iframe loading when needed", () => {
    const { container } = render(Embed, {
      props: { src: "https://example.com", title: "Hero map", loading: "eager" },
    });
...
describe("DataImage (FR2)", () => {
  it("renders a lazy img with src + required alt", () => {
    const { container } = render(DataImage, { props: { src: "/a.png", alt: "Aperçu" } });
  ...
  it("defaults fit to cover, applies contain when asked", () => {
    const cover = render(DataImage, { props: { src: "/a.png", alt: "a" } });
    expect(cover.container.querySelector("img")?.classList.contains("st-dataImage--cover")).toBe(true);
    const contain = render(DataImage, { props: { src: "/a.png", alt: "a", fit: "contain" } });
  ...
  it("maps numeric width/height/radius to px styles", () => {
    const { container } = render(DataImage, { props: { src: "/a.png", alt: "a", width: 120, height: 80, radius: 8 } });
  ...
  it("accepts CSS-length string dimensions verbatim", () => {
    const { container } = render(DataImage, { props: { src: "/a.png", alt: "a", width: "100%", radius: "50%" } });
    const img = container.querySelector<HTMLImageElement>("img");
  ...
  it("preserves caller style while applying dimensions", () => {
    const { container } = render(DataImage, {
      props: { src: "/a.png", alt: "a", width: 120, style: "object-position: center;" },
    });
    const img = container.querySelector<HTMLImageElement>("img");
    expect(img?.style.width).toBe("120px");
  ...
  it("allows eager loading and decoding overrides", () => {
    const { container } = render(DataImage, {
      props: { src: "/a.png", alt: "a", loading: "eager", decoding: "sync" },
    });
    const img = container.querySelector("img");
