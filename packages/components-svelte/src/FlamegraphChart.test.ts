...
import FlamegraphChart from "./lib/FlamegraphChart.svelte";

// Pile d'appels « icicle » : un nœud racine récursif, largeur ∝ value, enfants
// posés sous leur parent en se partageant la largeur.
const tree = {
  name: "root",
  value: 100,
  children: [
...
describe("FlamegraphChart", () => {
  it("renders one frame per node in the recursive tree", () => {
    const { container } = render(FlamegraphChart, { props: { label: "Profil", data: tree } });
    expect(screen.getByRole("img")).toBeTruthy();
    // root + a + a1 + a2 + b = 5 nœuds.
  ...
  it("makes a child width proportional to its value within the parent", () => {
    ...
    // a (60) plus large que b (40) ; les deux additionnés ≈ largeur racine.
  ...
  it("drops children with non-positive or non-finite values", () => {
    const { container } = render(FlamegraphChart, {
      props: {
        label: "Filtré",
        data: {
          name: "root",
