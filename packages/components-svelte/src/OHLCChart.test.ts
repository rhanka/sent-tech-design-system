...
import OHLCChart from "./lib/OHLCChart.svelte";
...
describe("OHLCChart", () => {
  it("renders one bar per period", () => {
    const { container } = render(OHLCChart, { props: { label: "Cours", data: week } });
  ...
  it("renders an open tick (left) and a close tick (right) per period", () => {
    ...
    // open tick goes from the left towards the center, close tick from the center to the right
    const open0 = opens[0];
  ...
  it("applies the up class when close ≥ open and the down class otherwise", () => {
    ...
    // Lun (index 0) haussier, Mer (index 2) baissier
  ...
  it("drops non-finite periods before rendering", () => {
    const { container } = render(OHLCChart, {
      props: {
        label: "Filtré",
        data: [
...
describe("OHLCChart — annotations / dataLabels / crosshair / keyboard nav", () => {
  ...
  it("renders a support line, a price region and a point annotation", () => {
    ...
    const { container } = render(OHLCChart, { props: { label: "Cours", data: week, annotations } });
  ...
  it("renders one close data label per bar (aria-hidden)", () => {
    const { container } = render(OHLCChart, { props: { label: "Cours", data: week, dataLabels: true } });
  ...
  it("draws a controlled crosshair at the bar matching hoverKey", async () => {
    const { container, rerender } = render(OHLCChart, { props: { label: "Cours", data: week, hoverKey: null } });
    expect(container.querySelector(".st-ohlcChart__crosshairLine")).toBeNull();
  ...
  it("enables a roving keyboard overlay announcing date + O/H/L/C", async () => {
    ...
    const { container } = render(OHLCChart, { props: { label: "Cours", data: week, onSelectKey } });
    ...
    await fireEvent.keyDown(datums[0], { key: "ArrowRight" });
    await fireEvent.keyDown(
      container.querySelectorAll<SVGRectElement>(".st-ohlcChart__navDatum")[1],
      { key: "Enter" }
    );
