...
import VennChart from "./lib/VennChart.svelte";
import type { VennChartArea } from "./lib/VennChart.svelte";
...
describe("VennChart", () => {
  it("renders one circle per set with role=img (2 sets)", () => {
    const { container } = render(VennChart, { props: { label: "Overlap", data } });
  ...
  it("renders three circles for a 3-set diagram", () => {
    const { container } = render(VennChart, { props: { label: "Triple", data: data3 } });
  ...
  it("maps radius by total set size (sqrt scaling): larger total yields a larger circle", () => {
    const { container } = render(VennChart, {
      props: {
        label: "Radius",
        data: [
          { sets: ["Small"], value: 1 },
  ...
  it("applies a category tone class per circle", () => {
    const { container } = render(VennChart, { props: { label: "Tone", data } });
  ...
  it("renders empty without crashing for an empty dataset", () => {
    const { container } = render(VennChart, { props: { label: "Empty", data: [] } });
  ...
  it("shows, updates, and hides tooltip on interactions", async () => {
    const { container } = render(VennChart, { props: { label: "Interactive", data } });

    expect(screen.queryByRole("presentation")).toBeNull();

    const circles = container.querySelectorAll(".st-vennChart__circle");
    await fireEvent.pointerMove(circles[1]);

    expect(screen.getByRole("presentation")).toBeTruthy();
    expect(container.querySelector(".st-vennChart__tooltipLabel")?.textContent).toBe("B");

    await fireEvent.pointerLeave(container.querySelector(".st-vennChart__visual") as Element);
    expect(screen.queryByRole("presentation")).toBeNull();
  });
});
