...
import ArcDiagramChart from "./lib/ArcDiagramChart.svelte";
...
describe("ArcDiagramChart", () => {
  it("renders one arc per valid link and one node marker per node", () => {
    const { container } = render(ArcDiagramChart, { props: { label: "Réseau", data: deps } });
  ...
  it("uses labels for display when provided", () => {
    const { container } = render(ArcDiagramChart, {
      props: {
        label: "Réseau",
