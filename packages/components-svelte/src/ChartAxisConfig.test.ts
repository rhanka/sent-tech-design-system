...
// FR1 — value-axis config (scale / invertAxis / domain / showLegend) reference.
...
describe("BarChart axis config (FR1)", () => {
  it("is unchanged without the new props (linear, 0 baseline)", () => {
    const { container } = render(BarChart, { props: { label: "Plain", data: barData } });
    expect(tickLabels(container, "barChart")).toContain("0");
  });

  it("scale='log' emits power-of-ten ticks (1,10,100) and no 0 baseline", () => {
    const { container } = render(BarChart, { props: { label: "Log", data: barData, scale: "log" } });
  ...
  it("scale='log' positions bars by log — decade steps equal pixel steps", () => {
    const { container } = render(BarChart, { props: { label: "Log", data: barData, scale: "log", height: 240 } });
    const [a, b, c] = bars(container).map((r) => Number(r.getAttribute("y")));
    const gap1 = a - b;
    const gap2 = b - c;
  ...
  it("scale='log' ignores non-positive values (no crash)", () => {
    ...
    const { container } = render(BarChart, { props: { label: "LogZero", data: withZero, scale: "log" } });
  ...
  it("scale='log' rejects a non-positive fixed domain and falls back to positive data", () => {
    const { container } = render(BarChart, {
      props: { label: "BadLogDomain", data: barData, scale: "log", domain: [0, 1000] },
    });
    const labels = tickLabels(container, "barChart");
    expect(labels).toContain("100");
  ...
  it("invertAxis flips the value axis (top tick value differs)", () => {
    ...
    const normal = render(BarChart, { props: { label: "N", data: barData } });
    const inverted = render(BarChart, { props: { label: "I", data: barData, invertAxis: true } });
    expect(topTick(normal.container)).not.toBe(topTick(inverted.container));
  });

  it("accepts showLegend without regression (no-op)", () => {
    const { container } = render(BarChart, { props: { label: "Legend", data: barData, showLegend: true } });
...
describe("LineChart axis config (FR1)", () => {
  it("honours a fixed domain (ticks include the bounds)", () => {
    const { container } = render(LineChart, { props: { label: "Dom", data: lineData, domain: [0, 200] } });
  ...
  it("keeps a non-nice fixed domain as the actual scale bounds", () => {
    const { container } = render(LineChart, { props: { label: "DomExact", data: lineData, domain: [3, 103] } });
    const labels = tickLabels(container, "lineChart");
  ...
  it("an invalid domain falls back to the auto range", () => {
    const { container } = render(LineChart, { props: { label: "Bad", data: lineData, domain: [5, 5] } });
    expect(tickLabels(container, "lineChart").length).toBeGreaterThan(0);
  });

  it("scale='log' emits power-of-ten ticks", () => {
    const { container } = render(LineChart, { props: { label: "Log", data: lineData, scale: "log" } });
    const labels = tickLabels(container, "lineChart");
    expect(labels).toContain("1");
    expect(labels).toContain("10");
    expect(labels).toContain("100");
  ...
  it("scale='log' + domain pins the log bounds", () => {
    const { container } = render(LineChart, { props: { label: "LogDom", data: lineData, scale: "log", domain: [1, 1000] } });
  ...
  it("invertAxis flips the data point order top-to-bottom", () => {
    ...
    const normal = render(LineChart, { props: { label: "N", data: lineData } });
    const inverted = render(LineChart, { props: { label: "I", data: lineData, invertAxis: true } });
    expect(firstDotY(normal.container)).toBeGreaterThan(firstDotY(inverted.container));
  });

  it("accepts showLegend without regression (no-op)", () => {
    const { container } = render(LineChart, { props: { label: "Legend", data: lineData, showLegend: true } });
