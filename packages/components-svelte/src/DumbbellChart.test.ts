...
import type { DumbbellChartDatum } from "./lib/DumbbellChart.svelte";
...
describe("DumbbellChart", () => {
  it("renders a connector line per category", () => {
    const { container } = render(DumbbellChart, { props: { label: "Salaires", data } });

    expect(screen.getByRole("img", { name: "Salaires" })).toBeTruthy();
  ...
  it("normalizes inverted low/high (lo <= hi)", () => {
    const { container } = render(DumbbellChart, {
  ...
  it("applies the low/high tone classes to the dots", () => {
    const { container } = render(DumbbellChart, {
      props: { label: "Tone", data, lowTone: "category3", highTone: "category5" },
    });
  ...
  it("renders a legend with the low/high labels", () => {
    const { getByText } = render(DumbbellChart, {
      props: { label: "Legend", data, lowLabel: "2019", highLabel: "2024" },
    });
  ...
  it("shows, updates, and hides tooltip on interactions", async () => {
    const { container } = render(DumbbellChart, { props: { label: "Interactive", data } });

    expect(screen.queryByRole("presentation")).toBeNull();

    const dots = container.querySelectorAll(".st-dumbbellChart__dot");
    await fireEvent.pointerMove(dots[2]); // second category (Design) low dot

    const tooltip = screen.getByRole("presentation");
    expect(tooltip).toBeTruthy();
    expect(screen.getAllByText("Design").length).toBeGreaterThan(0);
    expect(screen.getAllByText("54 – 69").length).toBeGreaterThan(0);

    await fireEvent.pointerLeave(container.querySelector(".st-dumbbellChart__visual") as Element);
    expect(screen.queryByRole("presentation")).toBeNull();
  });
});
