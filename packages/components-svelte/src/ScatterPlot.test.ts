...
import type { ScatterPlotDatum } from "./lib/ScatterPlot.svelte";
import type { ChartAnnotation } from "./lib/chartAnnotations.js";

// ScatterPlot "Highcharts-class" props (svelte), strict tri-framework parity:
...
describe("ScatterPlot annotations", () => {
  it("renders nothing extra by default (additive)", () => {
    const { container } = render(ScatterPlot, { props: { label: "Plain", data } });
  ...
  it("renders the 5 kinds (x AND y continuous) and appends a11y items", () => {
    ...
    const { container } = render(ScatterPlot, { props: { label: "Ann", data, annotations } });
  ...
  it("draws regions BEHIND and other annotations ABOVE the points", () => {
    ...
    const { container } = render(ScatterPlot, { props: { label: "Order", data, annotations } });
...
describe("ScatterPlot data labels (FR-2)", () => {
  ...
  it("`dataLabels={true}` shows the datum label (else the value) on every point", () => {
    const { container } = render(ScatterPlot, { props: { label: "DL", data, dataLabels: true } });
...
describe("ScatterPlot crosshair / synchronised hover (FR-3)", () => {
  it("uncontrolled: no crosshair by default, internal hover drives it (crossed pair)", async () => {
    const { container } = render(ScatterPlot, { props: { label: "Uncontrolled", data } });
    ...
    await fireEvent.pointerMove(points[1]);
    ...
    await fireEvent.pointerLeave(container.querySelector(".st-scatterPlot__visual") as Element);
    expect(container.querySelector(".st-scatterPlot__crosshair")).toBeNull();
  });

  it("controlled: crosshair + tooltip track hoverKey (the label), ignoring pointer", async () => {
    const { container } = render(ScatterPlot, { props: { label: "Controlled", data, hoverKey: "C" } });
    expect(container.querySelector(".st-scatterPlot__crosshair")).not.toBeNull();
    expect(container.querySelector(".st-scatterPlot__tooltipLabel")?.textContent).toBe("C");
    const points = container.querySelectorAll(".st-scatterPlot__point");
    await fireEvent.pointerMove(points[0]);
    expect(container.querySelector(".st-scatterPlot__tooltipLabel")?.textContent).toBe("C");
  });

  it("controlled with hoverKey=null shows nothing", () => {
    const { container } = render(ScatterPlot, { props: { label: "Null", data, hoverKey: null } });
    expect(container.querySelector(".st-scatterPlot__crosshair")).toBeNull();
  });

  it("emits onHoverKeyChange on move (the point key) and on leave (null)", async () => {
    const onHoverKeyChange = vi.fn();
    const { container } = render(ScatterPlot, { props: { label: "Emit", data, onHoverKeyChange } });
    const points = container.querySelectorAll(".st-scatterPlot__point");
    await fireEvent.pointerMove(points[2]);
...
describe("ScatterPlot — keyboard datapoint navigation (FR-5)", () => {
  ...
  it("overlay datums carry key + value labels in data order", () => {
    const { container } = render(ScatterPlot, { props: { label: "Nav", data, keyboardNav: true } });
  ...
  it("is enabled implicitly by wiring onSelectKey", () => {
    const { container } = render(ScatterPlot, { props: { label: "Nav", data, onSelectKey: () => {} } });
  ...
  it("Escape leaves the navigation (onSelectKey null) and resets the tab stop", async () => {
    ...
    await fireEvent.keyDown(navDatums(container)[2], { key: "Escape" });
