...
describe("ItemChart", () => {
  it("renders one seat per point (total = sum of values) inside a role=img", () => {
    const { container } = render(ItemChart, { props: { label: "Sièges", data: groups } });
  ...
  it("drops negative and non-finite values (zero seats for those groups)", () => {
    const { container } = render(ItemChart, {
      props: {
        label: "Filtré",
        data: [
          { label: "A", value: 10 },
    ...
    const all = seats(container);
    // Aucun siège pour B/C (index 1 et 2).
    const idx = all.map((s) => Number(s.getAttribute("data-chart-index")));
  ...
  it("renders nothing in the SVG when the total is zero, without crashing", () => {
    const { container } = render(ItemChart, {
      props: { label: "Vide", data: [{ label: "X", value: 0 }] },
    });
