...
import { buildArcDiagramData } from './arcDiagram.js';
...
describe('buildArcDiagramData', () => {
  it('maps source, target and weight fields per row', () => {
    const result = buildArcDiagramData(model, rows, config);
  ...
  it('skips rows where weight is non-finite', () => {
    ...
    const result = buildArcDiagramData(model, sparseRows, config);
  ...
  it('skips rows where weight is zero or negative', () => {
    ...
    const result = buildArcDiagramData(model, negativeRows, config);
    expect(result).toHaveLength(1);
  ...
  it('coerces source and target fields to strings', () => {
    ...
    const result = buildArcDiagramData(model, numericRows, config);
  ...
  it('coerces string weight values to numbers', () => {
    ...
    const result = buildArcDiagramData(model, stringWeightRows, config);
    expect(result).toHaveLength(1);
    expect(result[0]!.weight).toBe(6);
  });

  it('returns empty array for empty rows', () => {
    const result = buildArcDiagramData(model, [], config);
