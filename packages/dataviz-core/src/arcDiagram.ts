/**
 * Arc diagram chart data builder.
 *
 * Maps source, target and weight fields from raw rows into ArcDiagramChartLink-
 * compatible objects `{ from, to, weight }`, as expected by the DS
 * ArcDiagramChart component.
 *
 ...
 * ArcDiagramChart.svelte.d.ts: "seuls les liens dont `weight` est fini et > 0
 * sont pris en compte"). `from` and `to` are coerced to string.
 */
...
/**
 * Build arc diagram link data from raw rows by mapping source, target and
 ...
 */
export function buildArcDiagramData(
  _model: DataModel,
  rows: readonly Row[],
  config: ArcDiagramConfig,
): ArcDiagramLink[] {
  const data: ArcDiagramLink[] = [];
