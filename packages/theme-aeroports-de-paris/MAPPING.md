...
- **`motion.slow` `500ms`** — measured on `redirection-grille` and `frontTools .mentions` (the tooltip carries no transition); `fast 150ms` (links) and `normal 200ms` (buttons, accordion) are the better-supported steps.
...
- **Geometry disagreements (2024 squared vs current bundle)** — the 2024 sheet squares everything (`border-radius:0`), but the current bundle consumes 51 distinct `rounded-*` utilities, `.badgeTags{border-radius:4px}` sits against `tag.radius: 0`, and two fullpage CTAs are pills (`border-radius:100px`); the squared transcription stands on the brand-owned 2024 rules and the disagreement is recorded here. Likewise two fullpage hovers repaint petrol (`#004650` backgrounds) against the red-hover narrative — see Typography/Links.
...
- **Radius**: squared — `border-radius:0` on buttons, inputs, alerts, dropdowns (`radius.sm/md/lg = 0`); pills stay `999px` (measured `50%`/`100%` dots). Against: 51 distinct `rounded-*` utilities consumed by the current bundle, `.badgeTags{border-radius:4px}`, two fullpage pill CTAs (`border-radius:100px`) — disagreements recorded in À confirmer.
