...
const cases: Case[] = [
  ...
  },
  {
    ...
    expectedSignatureDiffs: 0,
    attribution:
      'exact match — the 5 Button aria-label/aria-pressed gaps are closed since the Angular DS Button forwards both like React/Vue (React rendering aria-pressed="false" explicitly; bare Button control is 0); variants, texts, disabled states and handlers all match',
    control: {
      ng: NGDS.Button as Type<unknown>,
      template: `<st-button variant="primary" size="sm" [aria-label]="'France'" [aria-pressed]="true">France</st-button>`,
      re: REDS.Button as ComponentType<Props>,
