...
| Rollout Phase 2 (~55 composants restants) | 🟡 | Garde `control.hoverBackground` ajoutée: le builder émet désormais `--st-component-control-hoverBackground` pour les composants hors pilotes qui le consomment (Accordion, Combobox, CopyButton, DataTable, DatePicker, FileUploader, NumberInput, PasswordInput, Search). |
...
| En cours | Rollout Phase 2 | Étendre les tokens partagés consommés hors pilotes | 🟡 | 20% | `control.hoverBackground` émis par tous les thèmes; garde TDD ajoutée dans `AnatomyTokens.test.ts`. |
