...
| G — A11y menus | Durcir clavier/focus au-dela du correctif rapide. | 90% | Nav clavier menu/overflow (Arrow/Home/End, Enter, skip disabled) + tests ; à valider sur popovers réels + focus trap si la surface l'exige. | Backlog DS |
...
| En cours | Rollout Phase 2 | Étendre les tokens partagés consommés hors pilotes | 🟡 | 40% | `control.hoverBackground` émis par tous les thèmes; élargissement de la consommation aux surfaces de survol de contrôles et de menus (IconButton, Menu, OverflowMenu, ContentSwitcher, Header, LanguageSelector, Toggletip, TreeView) + garde TDD consolidée dans `AnatomyTokens.test.ts`. |
