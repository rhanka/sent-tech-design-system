...
| WP10 | Theming via moteur design | 🟡 | 86% | Airbus public-ready sur `main`, comparatif local opérationnel, et garde-fous phase 2 étendus (control/field/pagination/sideNav/menu/dropdown/dataTable) en objet + CSS light/dark. | Poursuivre le rollout complet des composants hors pilotes + finaliser les retouches de finition visuelle ciblées. | Décision de distribution publique du thème Airbus (gouvernance de diffusion/validation produit). |
...
| En cours | Fidelity | Comparer les composants pilotes contre Airbus reel | 🟡 | 75% | Anatomie étendue + dark mode + alignements pilotes portés; run `node tools/compare/fidelity.mjs --date 2026-06-02 --fail-under 85` exécuté (92,4% global, 97 écarts nets, DSFR 91,1% / Carbon 93,0%). Vérifié 2026-06-02: build/check/test `packages/theme-airbus` OK, 3 tests verts. Reste référence visuelle redistribuable/pipeline final. |
| En cours | Rollout Phase 2 | Étendre les tokens partagés consommés hors pilotes | 🟡 | 74% | `control.hoverBackground` émis par tous les thèmes; élargissement de la consommation aux surfaces de survol de contrôles et de menus, plus `Switch`/`Toggle` (`hover` hors-checked + focus par stratégie). Bilan de consommation détaillé (status per composant) dans `docs/ds-theme-anatomy-matrix.md` (cluster formulaire + navigation/overlay). Airbus explicite désormais `button/input/link` et `tag`/`graph` en plus des blocs déjà couverts. Les `Choice` (`label`/`radio`/`checkbox`), `Search` et `Tabs` héritent explicitement des anatomies phase 2 dans les assertions tests (`packages/theme-airbus/src/index.test.ts`). `paginationNav`, `sideNav`, `menu`, `dropdown` et `dataTable` sont explicités dans les gardes de tokens émis (`packages/theme-airbus/src/index.test.ts`). |
...
| Attendu | Crédibilité | Basculer le thème change le **chrome entier** (header/logo/nav/sidebar), pas que la police | ✅ | 100% | Implémenté sur `main`; run de fidélité en clôture avec `node tools/compare/fidelity.mjs --date 2026-06-02 --fail-under 85` (92,4% global, DSFR 91,1% / Carbon 93,0%). |
