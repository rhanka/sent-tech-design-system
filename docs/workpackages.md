...
| WP10 | Theming via moteur design | 🟡 | 76% | DSFR/Carbon et Airbus public-ready sur `main`, comparatif local opérationnel, garde-fous tests phase2 Airbus ajoutés (`airbusTheme`). | Rollout Phase 2 des composants hors pilotes + gouvernance de publication client. | Décision de distribution publique du thème Airbus. |
...
| Fait | Contrat test | Proteger l'identite minimale du theme | 🟢 | 100% | Test Vitest red/green sur `compileTheme(airbusTheme)`/`airbusDarkTheme`, tokens clés + garde-explicite phase2 (hover/background/link). |
