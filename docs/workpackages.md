...
| En cours | Fidelity | Comparer les composants pilotes contre Airbus reel | 🟡 | 75% | Anatomie étendue + dark mode + alignements pilotes portés; run `node tools/compare/fidelity.mjs --date 2026-06-02 --fail-under 85` exécuté (92,4% global, 97 écarts nets, DSFR 91,1% / Carbon 93,0%). Vérifié 2026-06-02: build/check/test `packages/theme-airbus` OK, 3 tests verts, `apps/docs` compile/build OK. Environnement sandbox: le script fidélité complet ne peut être relancé ici car `listen` socket local est refusé (EPERM sur 127.0.0.1:4322). |
| En cours | Rollout Phase 2 | Étendre les tokens partagés consommés hors pilotes | 🟡 | 69% | `control.hoverBackground` émis par tous les thèmes; élargissement de la consommation aux surfaces de survol de contrôles et de menus, plus `Switch`/`Toggle` (`hover` hors-checked + focus par stratégie). Bilan de consommation détaillé (status per composant) dans `docs/ds-theme-anatomy-matrix.md` (cluster formulaire + navigation/overlay). `Choice`/`Search` explicités via assertions en tests compilés (`airbusTheme`). |
