...
| En cours | Rollout Phase 2 | Étendre les tokens partagés consommés hors pilotes | 🟡 | 48% | `control.hoverBackground` émis par tous les thèmes; élargissement de la consommation aux surfaces de survol de contrôles et de menus (Accordion, IconButton, Menu, OverflowMenu, ContentSwitcher, Header, LanguageSelector, Toggletip, TreeView, Notification, Tag, BackToTop) + garde TDD consolidée dans `AnatomyTokens.test.ts`. |
