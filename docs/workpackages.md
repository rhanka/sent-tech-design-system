...
| En cours | Rollout Phase 2 | Étendre les tokens partagés consommés hors pilotes | 🟡 | 53% | `control.hoverBackground` émis par tous les thèmes; élargissement de la consommation aux surfaces de survol de contrôles et de menus (Accordion, BackToTop, Combobox, ContentSwitcher, CopyButton, DataTable, DatePicker, Dropdown, FileUploader, Header, IconButton, LanguageSelector, Menu, MultiSelect, NumberInput, PaginationNav, Notification, OverflowMenu, PasswordInput, Search, Toggletip, Tag, TreeView) + garde TDD consolidée dans `AnatomyTokens.test.ts`. |
