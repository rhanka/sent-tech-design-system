...
- Notre sélecteur : `.cmp-scope--dsfr .st-switch__track`
- Référence : iframe `.cmp-cell--ref` → `.fr-toggle__label`
- ⚠ Le style DSFR est documenté via le même pattern Toggle. Pour comparer la surface toggle, on aligne la mesure sur le label (`.fr-toggle__label`).
...
**Fidélité Switch (dsfr) : 71%** — 22 `=`, 0 `~`, 9 `≠` sur 31 propriétés.
...
| padding-left | 16px | 16px | = |
| box width | 86.2px | 84.2px | ≠ Δ2.0px |
| box height | 38px | 36px | ≠ Δ2.0px |
...
**Fidélité Button (airbus) : 93.5%** — 25 `=`, 4 `~`, 2 `≠` sur 31 propriétés.
...
| padding-bottom | 6px | 6px | = |
| padding-left | 16px | 16px | = |
| box width | 92.4px | 90.4px | ≠ Δ2.0px |
| box height | 38px | 36px | ≠ Δ2.0px |
| font-family | Inter | Inter | = |
| font-size | 14px | 14px | = |
| font-weight | 700 | 700 | = |
...
**Fidélité ButtonDisabled (airbus) : 93.5%** — 25 `=`, 4 `~`, 2 `≠` sur 31 propriétés.
...
**Fidélité Input (airbus) : 100%** — 31 `=`, 0 `~`, 0 `≠` sur 31 propriétés.
...
**Fidélité InputError (airbus) : 100%** — 31 `=`, 0 `~`, 0 `≠` sur 31 propriétés.
...
**Fidélité InputDisabled (airbus) : 96.8%** — 30 `=`, 0 `~`, 1 `≠` sur 31 propriétés.
...
**Fidélité Textarea (airbus) : 90.3%** — 28 `=`, 0 `~`, 3 `≠` sur 31 propriétés.
...
**Fidélité Select (airbus) : 96.8%** — 30 `=`, 0 `~`, 1 `≠` sur 31 propriétés.
...
**Fidélité Search (airbus) : 100%** — 31 `=`, 0 `~`, 0 `≠` sur 31 propriétés.
...
- Notre sélecteur : `.cmp-scope--airbus .st-switch__track`
- Référence : iframe `.cmp-cell--ref` → `.ds-toggle-label`
- ⚠ Airbus does not expose a distinct Switch token set in this overlay scope; this maps to the same toggle label pattern for a comparable fidelity surface.
...
**Fidélité Switch (airbus) : 83.9%** — 26 `=`, 0 `~`, 5 `≠` sur 31 propriétés.
...
Couverture mesurée : **63 paires** (notre composant mappé vs vrai composant officiel).
...
| airbus | Button | 93.5% | 25 | 4 | 2 | oui |
| airbus | ButtonDisabled | 93.5% | 25 | 4 | 2 | oui |
| airbus | Input | 100% | 31 | 0 | 0 | oui |
| airbus | InputError | 100% | 31 | 0 | 0 | oui |
| airbus | InputDisabled | 96.8% | 30 | 0 | 1 | oui |
| airbus | Textarea | 90.3% | 28 | 0 | 3 | oui |
| airbus | Select | 96.8% | 30 | 0 | 1 | oui |
| airbus | Search | 100% | 31 | 0 | 0 | oui |
| airbus | Link | 64.5% | 20 | 0 | 11 | oui |
| airbus | Checkbox | 96.8% | 30 | 0 | 1 | oui |
| airbus | Radio | 96.8% | 30 | 0 | 1 | oui |
| airbus | Toggle | 83.9% | 26 | 0 | 5 | oui |
| airbus | Switch | 83.9% | 26 | 0 | 5 | oui |
...
- **Fidélité globale : 89.9%** (1721 `=`, 32 `~`, 198 `≠` sur 1951 propriétés mesurées).
- **Couverture : 63 paires composant×thème mesurées.**
- **Écarts nets restants : 198**
  ...
  - dsfr/Switch: radius topLeft (Δ999.0px)
  - dsfr/Switch: radius topRight (Δ999.0px)
  - dsfr/Switch: radius bottomRight (Δ999.0px)
  - dsfr/Switch: radius bottomLeft (Δ999.0px)
  - dsfr/Switch: box width (Δ-205.0px)
  - dsfr/Switch: box height (Δ-28.0px)
  - dsfr/Switch: line-height (normal vs 24px)
  - dsfr/Switch: background-color (rgb(0,0,145) vs rgb(0,0,0)/a0)
  - dsfr/Switch: color (rgb(58,58,58) vs rgb(22,22,22))
  ...
  - airbus/Button: box width (Δ2.0px)
  - airbus/Button: box height (Δ2.0px)
  - airbus/ButtonDisabled: box width (Δ2.0px)
  - airbus/ButtonDisabled: box height (Δ2.0px)
  ...
  - airbus/Switch: padding-left (Δ-48.0px)
  - airbus/Switch: box width (Δ-121.4px)
  - airbus/Switch: font-size (Δ2.0px)
  - airbus/Switch: line-height (normal vs 20px)
  - airbus/Switch: background-color (rgb(0,32,91) vs rgb(0,0,0)/a0)
