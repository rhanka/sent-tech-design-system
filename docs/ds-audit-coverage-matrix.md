...
| P2-1 dark-mode absent (cluster B) | `missing-dark-mode` | test positif (styles colorés sans `prefers-color-scheme`) + test négatif (`@media (prefers-color-scheme: dark)`) | **Couvert** |
...
Aucune finding priorisée majeure de `consolidated-v2.md` ne reste sans règle WP8.
