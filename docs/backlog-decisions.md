...
**Sous-décision liée (slots→snippets)** : l'API `<slot name="attachments|actions-left|actions-right">`
est migrée vers des snippets Svelte 5 (`attachments`, `actionsLeft`, `actionsRight`) dans `ChatComposer`,
car Svelte 5 interdit de mélanger `<slot>` et `{@render}`.
...
- T#9 sweep (2 agents A–M/N–Z) → **lot 1 corrigé** : 5 composants invisibles rendus visibles
  ...
  a11y `prefers-reduced-motion`, MessageStatusBadge (tokens docs + labels FR-only).
- T#9 sweep **lot 2** : AreaChart `@media (prefers-reduced-motion: reduce)` (tient la promesse a11y
  ...
  (migration vers `attachments` / `actionsLeft` / `actionsRight` effectuée).
